...
function physical_racks()
{
   ...
   if ($unknownID<-1) { krsort($racks); }
      ...
      while (list($rack,) = each($racks)) {
         ...
         foreach ($racks[$rack] as $name)
         {
             error_log("before: name ($name)");
         }
         arsort($racks[$rack], SORT_NUMERIC);
         foreach ($racks[$rack] as $name)
         {
             error_log("after: name ($name)");
         }
      }
   }
