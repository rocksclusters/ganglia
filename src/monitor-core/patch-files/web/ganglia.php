<?php
/* $Id: ganglia.php,v 1.1 2009/11/18 21:13:21 bruno Exp $ */
#
...
{
   // ROCKS
   // make this value the same as the default TN value (20)
   $TTL = 20;

   if ($host['TN'] and $host['TMAX']) {
      // ROCKS
      // the default TN is 20 seconds. we'll consider a node dead after we
      // haven't heard from it in 5 minutes
      if ($host['TN'] > $host['TMAX'] * 15)
         return FALSE;
         $host_up = FALSE;
   }
   else {      # The old method.
      if (abs($cluster["LOCALTIME"] - $host['REPORTED']) > (15*$TTL))
         return FALSE;
   }
   return TRUE;
}
